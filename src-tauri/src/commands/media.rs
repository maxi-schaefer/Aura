...
async fn run_get_now_playing_raw() -> Result<Option<MediaInfo>, String> {
    ...
    if let Ok(thumbnail_ref) = properties.Thumbnail() {
        if let Ok(stream) = thumbnail_ref.OpenReadAsync().map_err(|e| e.to_string())?.await {
            ...
            if size > 0 {
                ...
                let _ = stream.ReadAsync(&buffer, size, InputStreamOptions::None).map_err(|e| e.to_string())?.await;
                let reader = DataReader::FromBuffer(&buffer).map_err(|e| e.to_string())?;
                let mut bytes = vec![0u8; size as usize];
                let _ = reader.ReadBytes(&mut bytes);
                thumbnail_base64 = Some(format!("data:{};base64,{}", content_type, general_purpose::STANDARD.encode(bytes)));
            }
        }
    }
    ...
    Ok(Some(MediaInfo {
        ...
        is_playing,
        position: timeline.as_ref().map(|t| t.Position().unwrap_or_default().Duration / 10_000_000).unwrap_or(0) as u64,
        duration: timeline.as_ref().map(|t| t.EndTime().unwrap_or_default().Duration / 10_000_000).unwrap_or(0) as u64,
        source: session.SourceAppUserModelId().map(|s| s.to_string()).unwrap_or_default()
    }))
}
