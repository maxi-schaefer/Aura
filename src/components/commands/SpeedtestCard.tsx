...
export const SpeedtestCard = ({ ping = 0, download = 0, upload = 0, stage = "ping" }: SpeedtestCardProps) => {
    const size = 220;
    const center = size / 2;
    const strokeWidth = 6;
    ...
    const getCircleProps = (radius: number, value: number, max: number, colorId: string): SVGMotionProps<SVGCircleElement> => {
        ...
        return {
            ...
            animate: { strokeDashoffset: circumference - progress * circumference, opacity: 1 },
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-6 w-full max-w-md mx-auto bg-transparent">
            ...
            <div className="relative flex justify-center items-center h-56 bg-transparent">
                <svg width={size} height={size} className="-rotate-90 overflow-visible">
                    ...
                    <motion.circle {...getCircleProps(90, download, 1000, "grad-down")} strokeWidth={strokeWidth} strokeLinecap="round" />
                    <motion.circle {...getCircleProps(73, upload, 500, "grad-up")} strokeWidth={strokeWidth} strokeLinecap="round" />
                    <motion.circle {...getCircleProps(56, 100 - Math.min(ping, 100), 100, "grad-ping")} strokeWidth={strokeWidth} strokeLinecap="round" />
                </svg>
...
const MinimalStatItem = ({ label, value, unit, color }: MinimalStatItemProps) => (
