...
export const TimerCard = ({ initialMinutes }: TimerCardProps) => {
  ...
  // Handle global events (Toggle/Reset)
  useEffect(() => {
    const handleAction = (e: any) => {
      if (e.detail.type === "toggle") setIsActive(!isActive);
    ...
    return () => window.removeEventListener("timer-action", handleAction);
  }, [isActive]);
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 space-y-6 font-sans">
      ...
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="flex items-center gap-3 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.05]"
      >
