    ...
    const stepsContent: Record<number, JSX.Element> = {
        [-1]: (
            <motion.div key="intro" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col items-center gap-6">
                <div className="h-16" />
                <h1 className="text-xl  text-white tracking-tight">Welcome to Aura</h1>
                <p className="text-sm text-white/40 text-center max-w-80 leading-relaxed">
        ...
        0: (
            <motion.div key="name" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
                <div className="flex items-center gap-3">
                    <div className="size-6 rounded-md bg-white/5 border border-white/10 flex items-center justify-center text-[10px] text-white/40 font-mono">1</div>
                    <h2 className="text-sm  text-white/90">Personalize your instance</h2>
                </div>
                <div className="relative group">
                    <input
                        ...
                        className="w-full bg-white/3 border border-white/10 rounded-lg px-4 py-3 text-base text-white outline-none focus:border-white/20 focus:bg-white/5 transition-all"
                    />
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1 opacity-0 group-focus-within:opacity-100 transition-opacity">
                        <kbd className="px-1.5 py-0.5 rounded border border-white/10 bg-white/5 text-[9px] text-white/40 font-sans">Enter</kbd>
                    </div>
                </div>
            </motion.div>
        ),
        1: (
            <motion.div key="config" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-5">
                <div className="flex items-center gap-3">
                    <div className="size-6 rounded-md bg-white/5 border border-white/10 flex items-center justify-center text-[10px] text-white/40 font-mono">2</div>
                    <h2 className="text-sm  text-white/90">Choose Search Engine</h2>
                </div>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className={`text-sm ${isSelected ? "text-white " : "text-white/40"}`}>
                                        {eng.name}
                                    </span>
                                </div>
                                {isSelected && (
                ...
                <button
                    onClick={() => { setStep(2); handleFinalize(); }}
                    className="w-full flex items-center justify-center py-3 bg-white/5 cursor-pointer border border-white/10 text-white rounded-lg hover:bg-white/10 transition-all active:scale-[0.98] text-xs "
                >
                    Finalize Setup
                </button>
            </motion.div>
        ...
        2: (
            <motion.div key="init" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col items-center py-10 space-y-4">
                ...
                <div className="text-center">
                    <p className="text-sm  text-white/80">Initializing Aura...</p>
                    <p className="text-[11px] text-white/30 mt-1">Setting up environment for {username}</p>
                </div>
            </motion.div>
        ),
    };
    ...
    return (
        <div className="h-screen w-full flex flex-col items-center justify-center glass overflow-hidden font-sans relative px-8">
            ...
            >
                ...
                {step !== -1 && <span className="text-[10px] font-mono text-white/20 uppercase tracking-widest">Setup</span>}
            </motion.div>
            ...
            <div className="absolute bottom-6 w-full px-10 flex justify-between items-center opacity-20">
                <span className="text-[10px] font-mono text-white tracking-tight italic">v0.2.6</span>
                <div className="flex gap-4">
                    <span className="text-[9px] text-white uppercase tracking-tighter">ESC to Quit</span>
                    <span className="text-[9px] text-white uppercase tracking-tighter">Enter to Select</span>
                </div>
            </div>
        </div>
    );
